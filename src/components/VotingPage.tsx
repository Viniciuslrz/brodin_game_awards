...
export const VotingPage = ({
  ...
  // Reset the game selections whenever the component mounts
  useEffect(() => {
    setGames(initialGames.map((game) => ({ ...game, check: false })));
  }, [initialGames]);
  ...
  return (
    <div
      className={`w-5/6 rounded-md my-2 pb-2 flex flex-col justify-self-center ${
        pagenumber === 1
          ? "bg-blue-300"
          : pagenumber === 2
          ? "bg-yellow-300"
